package array;
import java.util.Scanner;
public class _04_Invertir_Array {
      public static void main(String[] args) {
            //Escribe un programa que solicite al usuario
            //ingresar el tamaño de un array de números enteros.
            //A continuación se le pedirá números hasta que el array esta lleno.
            //El siguiente paso será invertir el orden de los valores  e imprimirlos por pantalla.
            Scanner sc = new Scanner(System.in);
            
            int num;
            int mayor = 0;

            System.out.println("Ingresa el tamaño del array");
            int tamaño = sc.nextInt();
            int[] num1 = new int [tamaño];

            for(int i = 0; i < tamaño; i++) {
                  System.out.println("Ingresa los números: ");
                  num1[i] = sc.nextInt();

                  }

            for(int i = 0; i < tamaño / 2; i++) {
                  int resultado = num1[i];
                  num1[i] = num1[tamaño - 1 - i];
                  num1[tamaño - 1 - i] = resultado;


            }

            System.out.println("array invertido");
            for(int con : num1)
            System.out.println(con);

      }

}